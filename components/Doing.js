...
class Doing extends Component {
  ...
  renderList = (item) => {
    const end = new Date(Date.parse(item.item.dataItem.endDate));
    ...
    return (
      <TouchableOpacity onPress={() => this.props.detailTask(item.item.dataItem, {item: item.item.dataItem})} activeOpacity={1}>
        <ListComponent style={styles.list}>
          ...
          <Text style={{ ...styles.contentList, alignSelf: 'flex-start', fontSize: 12, flex: 3, marginTop: 24, color: end < now ? 'red' : colour.primary }}>{item.item.dataItem.endDate}</Text>
          {
  ...
  render() {
    ...
    var doing = this.props.todoList.filter(item => item.status == 'Doing' && item.priority == priority);
    return (
      <View style={{flex: 1}}>
        <VirtualizedList
